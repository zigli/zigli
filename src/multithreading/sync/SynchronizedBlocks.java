...
public class SynchronizedBlocks {
    ...
    private void copy() {
        ...
        System.out.println(Thread.currentThread().getName() + " = " + "Start time:" + start);
        ...
        System.out.println(Thread.currentThread().getName() + " = " + "Stop time:" + end);
    ...
    private void copyArrayA() {
        synchronized (lock1) {
            for (int i = 0; i < a.length; i++) {
                intList1.add(a[i]);
                System.out.println(Thread.currentThread().getName() + " = " + intList1);
                try {
                    Thread.sleep(100);
                } catch (InterruptedException e) {
                    e.printStackTrace();
    ...
    private void copyArrayB() {
        synchronized (lock2) {
            for (int i = 0; i < b.length; i++) {
                intList2.add(b[i]);
                System.out.println(Thread.currentThread().getName() + " = " + intList2);
                try {
                    Thread.sleep(100);
                } catch (InterruptedException e) {
                    e.printStackTrace();
